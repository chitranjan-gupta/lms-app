...
import { setJob } from "@/core/store/job";
import { bookmarkRow } from "@/core/store/rows";
...
export const JobCard = ({ item }: JobCardProps) => {
  const onPress = () => {
    setJob({ jobId: item.$id });
    router.push("/(auth)/(jobs)");
  };
  const onBookmark = () => {
    bookmarkRow({ title: item.title, subtitle: item.company.name, job: item });
  };
  return (
    <View
      key={item.$id}
